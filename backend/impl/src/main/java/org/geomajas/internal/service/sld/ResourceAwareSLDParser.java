...
package org.geomajas.internal.service.sld;
 ...
 * 
 */
public class ResourceAwareSLDParser {
	 ...
	 */
	public ResourceAwareSLDParser(StyleFactory factory, ResourceService resourceService) {
	 ...
	 * @param factory The StyleFactory to use to build the style
	 */
	public ResourceAwareSLDParser(StyleFactory factory) {
		this(factory, CommonFactoryFinder.getFilterFactory(GeoTools.getDefaultHints()));
	}

	public ResourceAwareSLDParser(StyleFactory factory, FilterFactory filterFactory) {
	 ...
	 */
	public ResourceAwareSLDParser(StyleFactory factory, String filename) throws java.io.FileNotFoundException {
		this(factory);
	 ...
	 * @throws java.io.FileNotFoundException - if the file is missing
	 */
	public ResourceAwareSLDParser(StyleFactory factory, File f) throws java.io.FileNotFoundException {
		this(factory);
		setInput(f);
	}
	 ...
	 */
	public ResourceAwareSLDParser(StyleFactory factory, java.net.URL url) throws java.io.IOException {
		this(factory);
		setInput(url);
	}
	 ...
	 */
	public ResourceAwareSLDParser(StyleFactory factory, java.io.InputStream s) {
		this(factory);
		setInput(s);
	}
	 ...
	 */
	public ResourceAwareSLDParser(StyleFactory factory, java.io.Reader r) {
		this(factory);
		setInput(r);
	}
	...
	/**
	 * adds the key/value pair from the node ("<VendorOption name="...">...</VendorOption>"). This can be generalized
	...
	protected ColorMapEntry parseColorMapEntry(Node root) {
		...
		if (atts.getNamedItem(colorMapQuantityString) != null) {
			symbol.setQuantity(ff.literal(atts.getNamedItem(colorMapQuantityString).getNodeValue()));
		}
	...
	protected SelectedChannelType parseSelectedChannel(Node root) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (childName == null) {
				 ...
				 * .parse(child.getFirstChild().getNodeValue())); } catch (Exception e) { // TODO: handle exception }
				 */
			}
		}
	...
	protected ExternalGraphic parseExternalGraphic(Node root) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (childName.equalsIgnoreCase("format")) {
				...
				if (LOGGER.isLoggable(Level.FINEST))
					LOGGER.finest("seting custom property " + propName + " to " + getFirstChildValue(child));
		...
		if (resourceService != null) {
			try {
				...
				LOGGER.warning("can't parse " + uri + " as resource");
			}
		}
