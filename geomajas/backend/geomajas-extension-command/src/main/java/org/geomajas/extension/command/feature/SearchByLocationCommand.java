...
public class SearchByLocationCommand implements Command<SearchByLocationRequest, SearchByLocationResponse> {
	...
	public void execute(SearchByLocationRequest request, SearchByLocationResponse response) throws Exception {
		...
		if (layerIds != null && layerIds.length > 0) {
			...
			// Go over all requested layers:
			for (String layerId : layerIds) {
				Layer<?> layer = runtimeParameters.getLayer(layerId);
				...
				if (layer instanceof VectorLayer) {
					...
					/*
					 * @todo security, add data filter if (filters != null && filters[i] != null) { f =
					 * FilterCreator.createLogicFilter(f, "and", filters[i]); }
					 */

					// Create a featurePainter, and search according to the filter:
					FeaturePainter featurePainter = paintFactory.createFeaturePainter();
					vectorLayer.paint(featurePainter, f, null, runtimeParameters.getCrs(request.getCrs()));
					List<RenderedFeature> temp = featurePainter.getFeatures();
					if (temp.size() > 0) {
						...
						if (queryType == SearchByLocationRequest.QUERY_INTERSECTS && ratio > 0 && ratio <= 1) {
							for (RenderedFeature feature : temp) {
								try {
									...
									// Don't add the feature, just try the next? Perhaps we should simply stop this
									// command?
								}
							}
						} else {
