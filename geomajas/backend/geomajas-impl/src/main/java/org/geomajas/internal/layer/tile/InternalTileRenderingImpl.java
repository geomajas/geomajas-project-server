/*
 * This file is part of Geomajas, a component framework for building
 * rich Internet applications (RIA) with sophisticated capabilities for the
 * display, analysis and management of geographic information.
 * It is a building block that allows developers to add maps
 * and other geographic data capabilities to their web applications.
 *
 * Copyright 2008-2010 Geosparc, http://www.geosparc.com, Belgium
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */

package org.geomajas.internal.layer.tile;

import org.geomajas.layer.tile.InternalTileRendering;

/**
 * <p>
 * The actually rendered representation of a tile.
 * </p>
 * 
 * @author Pieter De Graef
 */
public class InternalTileRenderingImpl implements InternalTileRendering {

	private TileRenderMethod tileRenderMethod;

	private String featureRendering;

	private String labelRendering;

	//-------------------------------------------------------------------------
	// Constructors:
	//-------------------------------------------------------------------------

	public InternalTileRenderingImpl(TileRenderMethod tileRenderMethod) {
		this.tileRenderMethod = tileRenderMethod;
	}

	//-------------------------------------------------------------------------
	// TileRendering implementation:
	//-------------------------------------------------------------------------

	/**
	 * @return Return the method used in rendering a tile. Depending on the value, this object will contain a different
	 *         content.
	 */
	public TileRenderMethod getTileRenderMethod() {
		return tileRenderMethod;
	}

	/**
	 * Return the rendering of a tile's features. Depending on the rendering method used, the returned string will
	 * contain an entire rendering (SVG/VML) or a URL.
	 */
	public String getFeatureRendering() {
		return featureRendering;
	}

	/**
	 * Set the rendering of a tile's features. Depending on the rendering method used, the given string must contain an
	 * entire rendering (SVG/VML) or a URL.
	 * 
	 * @param featureString
	 *            The rendering of a tile's features.
	 */
	public void setFeatureRendering(String featureRendering) {
		this.featureRendering = featureRendering;
	}

	/**
	 * Return the rendering of a tile's labels. Depending on the rendering method used, the returned string will contain
	 * an entire rendering (SVG/VML) or a URL.
	 */
	public String getLabelRendering() {
		return labelRendering;
	}

	/**
	 * Set the rendering of a tile's labels. Depending on the rendering method used, the given string must contain an
	 * entire rendering (SVG/VML) or a URL.
	 * 
	 * @param labelString
	 *            The rendering of a tile's labels.
	 */
	public void setLabelRendering(String labelRendering) {
		this.labelRendering = labelRendering;
	}
}
