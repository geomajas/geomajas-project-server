dojo.provide("geomajas.gfx.painter.LabeledTilePainter");
/*
...
dojo.declare("LabeledTilePainter", Painter, {
	...
	paint : function (tile, graphics) {
		//log.info("RenderedTilePainter.paint: "+tile.getId());
		...
		if (tile.getFeatureImage() != null) {
			try {
				...
				if (tile instanceof LabeledTile) {
					graphics.drawImage (tile.getLabelImage(), {
						id : this._createLabelId(tile),
						style: tile.getLabelImage().getStyle(), 
						width : tile.getScreenWidth(),
						height : tile.getScreenHeight(),
						transform : transformationMatrix
					});
