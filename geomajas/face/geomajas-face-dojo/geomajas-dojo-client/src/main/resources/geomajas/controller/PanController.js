...
dojo.declare("PanController", MouseListener, {
	...
	mouseReleased : function (/*HtmlMouseEvent*/event) {
		try {
			...
			} else if (this.dragging) { // It is possible to MouseDown somewhere outside the SVG, and MouseUp here...
				this.dragging = false;
				this.mapWidget.setCursor("default");
				this._updateView (event);
	
				if (this.hideLabelsOnDrag == "true") {
					this.mapWidget.getGraphics().unhide("featureLabels");
				}
			}
		} catch (e){
	...
	mouseExited : function (/*HtmlMouseEvent*/event) {
		if (this.dragging) {
			...
			if (mouseX <= 0 || mouseY <= 0 || mouseX >= coords.w || mouseY >= coords.h) {
				log.debug("stopping pan");
				event.setPosition(this.begin); // coordinates of outside element are prolly not the same as in view
	...
	_updateView : function (event, status) {
        ...
        log.debug("PanController : _updateView dx "+c2.getX() - c1.getX()+" dy "+c2.getY() - c1.getY());
