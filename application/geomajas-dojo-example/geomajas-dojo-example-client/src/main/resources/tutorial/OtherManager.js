 ...
 */
dojo.provide("tutorial.OtherManager");

dojo.declare("OtherManager", ConfigManager, {
	...
	onConfigDone : function () {
		...
		var mapWidget = dijit.byId("sampleOtherMap");
		...
		var toolbar = dijit.byId("sampleOtherToolbar");
		...
		var layer = mapWidget.getMapModel().getLayerById("sampleOtherMap.cities");
		...
		// OverviewMap
		var overviewMap = dijit.byId("sampleOtherOverviewMap");

		if (overviewMap && mapWidget && mapWidget.id != "sampleOtherOverviewMap" /* Dont set to yourself! */) {
			overviewMap.setOverviewReference(mapWidget.id);
            overviewMap._onTargetMapRender(null, "all");
           	dojo.style(dojo.byId("sampleOtherOverviewMap"), 'display', 'block');
	...
	*/
	...
	openLayerStyle : function() {
		...
		var map = dijit.byId("sampleOtherMap");
