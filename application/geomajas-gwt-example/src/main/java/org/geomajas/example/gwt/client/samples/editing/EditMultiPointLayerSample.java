...
public class EditMultiPointLayerSample extends SamplePanel {
	...
	public Canvas getViewPanel() {
		...
		final MapWidget map = new MapWidget("editMultiPointLayerMap", "gwt-samples");
		map.getMapModel().addMapModelHandler(new MapModelHandler() {

			// When the map is initialized: select the cities layer - so that new features are created in this layer:
			public void onMapModelChange(MapModelEvent event) {
				map.getMapModel().selectLayer(map.getMapModel().getLayer("editableMultiCitiesLayer"));
			}
		});
	...
	public String[] getConfigurationFiles() {
		return new String[] { "classpath:org/geomajas/example/gwt/clientcfg/editing/mapEditMultiPointLayer.xml",
				"classpath:org/geomajas/example/gwt/servercfg/raster/layerWmsBluemarble.xml",
				"classpath:org/geomajas/example/gwt/servercfg/vector/layerMultiCities.xml" };
	}
