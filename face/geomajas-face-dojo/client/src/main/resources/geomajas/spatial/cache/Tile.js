...
dojo.declare("Tile", SpatialNode, {
	...
	fetch : function (filter, callback) {
		...
		command.addParam("y", this.code.getY());
		command.addParam("tileLevel", this.code.getTileLevel());
		...
		command.addParam("origX", this.cache.layer.getMapModel().getMapView().getPanOrigin().getX());
		command.addParam("origY", this.cache.layer.getMapModel().getMapView().getPanOrigin().getY());
	...
	/**
	 * creates the label id for this tile
	 */
	createLabelId : function () {
		return this.id.replace("features", "labels");
	},
