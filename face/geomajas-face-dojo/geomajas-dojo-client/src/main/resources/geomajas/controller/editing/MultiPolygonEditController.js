...
dojo.provide("geomajas.controller.editing.MultiPolygonEditController");
dojo.require("geomajas.event.MouseListener");
dojo.require("geomajas.action.menu.editing.ToggleGeomInfoAction");
...
dojo.declare("MultiPolygonEditController", MouseListener, {
	...
	constructor : function (parent, mapWidget) {
		...
		//DVB: let each controller have it's own menu
		//this.menuId = this.parent.menuId;
		this.menuId = "MultiPolygonEditMenu";
	...
	getName : function () {
		return "MultiPolygonEditController";
	},
	...
	getStartIndex : function (multipolygon) {
		return [0, 0];
	},

	getNewRingIndex : function (multipolygon) {
		var numHoles = multipolygon.getGeometryN(0).getNumInteriorRing();
		return [0, numHoles+1];
	},

	getNewPolygonIndex : function () {
		alert("MultiPolygonEditController._getNewPolygonIndex => implement me!!");
	},
	...
	mousePressed : function (/*HtmlMouseEvent*/event) {
		...
		if (this.parent.isDragging() && event.getButton() != event.statics.RIGHT_MOUSE_BUTTON && trans.isVertex(event.getTargetId())){
	...
	_checkAppendGeometryValidity : function (featureTransaction, index, coordinate) {
		var multiPolygon = featureTransaction.getNewFeatures()[index[0]].getGeometry().clone();
		// if it's a hole
		if (index[1].length == 2 && index[1][1] > 0) {
			var mathLib = new MathLib();
			if (!mathLib.isWithin(multiPolygon, coordinate)) {
				log.warn ("Point lies outside the multipolygon!");
				return false;
			}

			if (mathLib.touches(multiPolygon, coordinate)) {
				log.warn ("Point touches the multipolygon!");
				return false;
			}
		}
		// check self-intersection of linear ring (excluding last point)
		var geometry = multiPolygon.getGeometryN(index[1]);
		geometry.appendCoordinate(coordinate);
		return geometry.isNotIntersecting();
	},
	...
	_checkMoveGeometryValidity : function (featureTransaction, index, coordinate) {
		...
		this.editor.edit(multiPolygon, operation);
