...
public class AbstractAttributeForm {
	 ...
	 * @throws Exception
	 */
	public void toForm(String name, Attribute<?> attribute) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void fromForm(String name, Attribute<?> attribute) throws Exception {
		if (attribute == null) {
			return;
		}
		AttributeInfo info = infos.get(name);
		if (info instanceof PrimitiveAttributeInfo) {
			...
			switch (primitive.getType()) {
				...
				case DATE:
					...
					break;
				default:
					throw new Exception("Wrong attribute datatype");
			}
		} else {
