...
/**
 * <p> Started from {@link org.geomajas.gwt.client.widget.FeatureAttributeWindow}.
 * But this class extends Canvas instead of Window. 
 * Note that it has only been tested in read-only attribute window mode (no editing).   
 * 
 * The <code>FeatureAttributeCanvas</code> is a floating window that uses a
 * {@link org.geomajas.widget.featureinfo.client.widget.FeatureAttributeEditor} to change the alpha-numerical 
 * attributes of a feature and persist these changes on the server. In essence, this widget is a
 * {@link org.geomajas.widget.featureinfo.client.widget.FeatureAttributeEditor} with some extra buttons. One of 
 * these extra buttons is a "save" button to actually save the widget. When setting a feature, the underlying
 * {@link org.geomajas.widget.featureinfo.client.widget.FeatureAttributeEditor} automatically creates a clone. 
 * That way you're not editing the feature directly, and changes are only applied when the save is clicked. 
 * This widget will also check whether or not all fields are valid, and will not allow saving when at least one 
 ...
 */
@Api
public class FeatureAttributeCanvas extends Canvas {
	 ...
	 */
	@Api
	public FeatureAttributeCanvas(Feature feature, boolean editingAllowed, int maxHeight) {
		super();
		setEditingAllowed(editingAllowed);
		if (feature != null) {
			buildWidget(feature.getLayer());
			setFeature(feature, maxHeight);
		}
	}
	 ...
	 *
	 * @param feature
	 *            feature (if null do nothing)
	 */
	public void setFeature(Feature feature, final int maxHeight) {
		if (feature == null) {
			return;
		}
		...
		LazyLoader.lazyLoad(features, GeomajasConstant.FEATURE_INCLUDE_ATTRIBUTES, new LazyLoadCallback() {
			...
			public void execute(List<Feature> response) {
				Feature feature = response.get(0); // throws an exception if no features
				buildWidget(feature.getLayer());
	...
	private void buildWidget(VectorLayer layer) {
		if (layout != null && layerId != null && layerId.equals(layer.getId())) {
			return; /* DO NOTHING! */
		}
		if (layout != null) {
			layout.destroy();
