...
/**
 * Basic container component for printing. Handles the size calculation, layout
 * and rendering of its children.
 *
...
public abstract class BaseComponent implements PrintComponent {
	...
	/**
	 * Calculates the size based constraint width and height if present,
	 * otherwise from children sizes.
	 */
	...
	public String toString() {
		return getClass().getSimpleName() + " [" + (int) getBounds().getLeft() + " : "
				+ (int) getBounds().getRight() + "] x [" + (int) getBounds().getBottom() + " : "
				+ (int) getBounds().getTop() + "]";
	}
	...
	/**
	 * Resets cyclic references like child -> parent relationship Unfortunately,
	 * this has to be implemented in the concrete class !
	 *
	 * @param u
