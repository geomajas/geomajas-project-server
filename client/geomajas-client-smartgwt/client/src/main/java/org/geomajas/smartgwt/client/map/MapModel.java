...
import org.geomajas.smartgwt.client.map.event.HasFeatureSelectionHandlers;
...
import org.geomajas.smartgwt.client.map.event.MapModelEvent;
import org.geomajas.smartgwt.client.map.event.MapModelHandler;
...
import org.geomajas.smartgwt.client.map.feature.FeatureEditor;
			...
			if (featureCount == 0) {
				if (bounds != null) {
					if (bounds.getWidth() > 0 || bounds.getHeight() > 0) {
						getMapView().applyBounds(bounds, MapView.ZoomOption.LEVEL_FIT);
					} else {
						getMapView().setCenterPosition(bounds.getCenterPoint());
						getMapView().setCurrentScale(pointScale, MapView.ZoomOption.LEVEL_CLOSEST);
					}
				}
			}
		}
